...
export default class Game {
    constructor(app, loadedAssets) {
        ...
        this.config.baseWidth = this.config.symbolSize * 5 + this.config.symbolSpacing * 4 + this.config.mainPadding * 2;
        this.config.baseHeight = this.config.symbolSize * 5 + this.config.symbolSpacing * 4 + this.config.mainPadding * 2;
        ...
        this.ui = new GameUI(this.app, this.config, this.textContainer, this.buttonContainer);
        this.reels = [];
        this.paylines = null;
        this.scale = 1;
    ...
    async loadConfigs() {
        ...
        console.log(`🎮 Slot machine loaded: ${reels.length} reels, ${Object.keys(symbols).length} symbols, ${paylines.length} paylines`);
        
        SymbolFactory.setSymbolConfig(symbols);
        this.reels = reels.map((keys) => new Reel(keys, this.app, this.config.visibleCount, this.config.symbolSpacing));
    ...
    _positionContainers() {
        const slotContainerHeight = this.config.symbolSize * 3 + this.config.symbolSpacing * 2;
        const slotContainerWidth = 5 * (this.config.symbolSize + this.config.symbolSpacing) - this.config.symbolSpacing;
        const pad = this.config.mainPadding;
        
        // positionnement du conteneur de slot au centre
        this.slotContainer.pivot.set(slotContainerWidth / 2, slotContainerHeight / 2);
        ...
        const lastReelX = this.config.baseWidth / 2 + (slotContainerWidth / 2) - (this.config.symbolSize + this.config.symbolSpacing);
        const buttonY = this.config.baseHeight / 2 + slotContainerHeight / 2 + this.config.symbolSpacing / 4;
    ...
    calculateWinnings() {
        ...
        const winResults = this.paylines.calculateAllWins(visibleSymbols, SymbolFactory.getSymbolConfig());
