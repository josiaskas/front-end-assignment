...
export default class ConfigLoader {
    static validateReels(reels) {
        if (!Array.isArray(reels) || reels.length !== 5) {
            throw new Error('Reels configuration must be an array with exactly 5 bands');
        }
        ...
        reels.forEach((band, index) => {
            ...
            band.forEach((symbol, symbolIndex) => {
                if (typeof symbol !== 'string' || symbol.trim() === '') {
                    throw new Error(`Invalid symbol at position ${symbolIndex} in reel band ${index + 1}: "${symbol}"`);
                }
            });
        });
    }
        ...
        Object.entries(symbols).forEach(([symbolId, symbolConfig]) => {
            ...
            requiredFields.forEach(field => {
                if (!(field in symbolConfig)) {
                    throw new Error(`Symbol "${symbolId}" is missing required field: "${field}"`);
                }
            });

            if (typeof symbolConfig.textureKey !== 'string' || symbolConfig.textureKey.trim() === '') {
                throw new Error(`Symbol "${symbolId}" must have a valid textureKey string`);
            }

            if (typeof symbolConfig.payout !== 'object' || symbolConfig.payout === null) {
            ...
            requiredPayouts.forEach(payoutKey => {
                if (!(payoutKey in symbolConfig.payout)) {
                    throw new Error(`Symbol "${symbolId}" is missing payout for ${payoutKey} of a kind`);
                }
                if (typeof symbolConfig.payout[payoutKey] !== 'number' || symbolConfig.payout[payoutKey] < 0) {
                    throw new Error(`Symbol "${symbolId}" payout for ${payoutKey} of a kind must be a non-negative number`);
                }
            });
        });
    }
    ...
    static validateSymbolReferences(reels, symbols) {
        ...
        reels.forEach((band, bandIndex) => {
            band.forEach((symbol, symbolIndex) => {
                if (!validSymbols.includes(symbol)) {
                    throw new Error(`Reel band ${bandIndex + 1} position ${symbolIndex + 1} references undefined symbol: "${symbol}"`);
                }
        ...
        const unusedSymbols = validSymbols.filter(symbol => !usedSymbols.has(symbol));
        if (unusedSymbols.length > 0) {
            console.warn(`Warning: The following symbols are defined but not used in reels: ${unusedSymbols.join(', ')}`);
        }
    }
