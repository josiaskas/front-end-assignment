export default class Paylines {
    ...
    validatePaylines() {
        ...
        this.paylines.forEach((payline, paylineIndex) => {
            if (!Array.isArray(payline) || payline.length !== 5) {
                throw new Error(`Payline ${paylineIndex + 1} must be an array with exactly 5 positions`);
            }

            payline.forEach((symbolIndex, reelIndex) => {
                if (typeof symbolIndex !== 'number' || symbolIndex < 0 || symbolIndex > 2) {
                    throw new Error(`Payline ${paylineIndex + 1} reel ${reelIndex + 1} must be a number between 0 and 2`);
                }
            });
        });
    }
    ...
    getPaylineSymbols(visibleSymbols, paylineIndex) {
        ...
        return payline.map((symbolIndex, reelIndex) => visibleSymbols[reelIndex][symbolIndex]);
    }
    ...
    calculateAllWins(visibleSymbols, symbolConfig) {
        ...
        for (let i = 0; i < this.paylines.length; i++) {
            ...
            if (consecutiveCount >= 3 && symbolConfig[firstSymbol]) {
                const payout = symbolConfig[firstSymbol].payout[consecutiveCount.toString()];
                if (payout && payout > 0) {
                    totalWins += payout;
                    winDetails.push(`- payline ${i + 1}, ${firstSymbol} x${consecutiveCount}, ${payout}`);
                }
            }
        }
